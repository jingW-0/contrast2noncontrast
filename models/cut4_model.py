...
class CUT4Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_SRC', type=float, default=0, help='weight for SRC loss: SRC(G(X), X)')
        # parser.add_argument('--dce_idt', action='store_true')
        parser.add_argument('--dce_idt', type=bool, default=True)
    ...
    def compute_G_loss(self):
        ...
        ## HDCE
        if self.opt.lambda_NCE > 0.0:
            self.loss_NCE = self.calculate_NCE_loss(real_A_pool, fake_B_pool, weight)
        else:
            self.loss_NCE, self.loss_NCE_bd = 0.0, 0.0

        self.loss_NCE_Y = 0
        if self.opt.dce_idt and self.opt.lambda_NCE > 0.0:
            _, weight_idt = self.calculate_R_loss(real_B_pool, idt_B_pool, only_weight=True, epoch=self.train_epoch)
            self.loss_NCE_Y = self.calculate_NCE_loss(real_B_pool, idt_B_pool, weight_idt)
            loss_NCE_both = (self.loss_NCE + self.loss_NCE_Y) * 0.5
        else:
            loss_NCE_both = self.loss_NCE

        self.loss_G = self.loss_G_GAN + loss_NCE_both + self.loss_SRC
        return self.loss_G


    def calculate_NCE_loss(self, src, tgt, weight=None):
        ...
        for i, (f_q, f_k, crit, nce_layer, w) in enumerate(zip(feat_q_pool, feat_k_pool, self.criterionNCE, self.nce_layers, weight)):
            # if self.opt.no_Hneg or i < len(weight) - 1:
            if self.opt.no_Hneg:
                w = None

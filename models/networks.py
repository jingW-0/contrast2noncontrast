...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    elif netF == 'global_sample':
        net = GlobalSampleF(use_mlp=opt.netF2_mlp, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
    elif netF == 'global_cluster':
        net = GlobalClusterF(use_mlp=opt.netF2_mlp, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc, km_mode=opt.kmeans_mode, opt=opt)
    elif netF == 'global_cluster2':
        net = GlobalClusterF2(use_mlp=opt.netF2_mlp, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids,
...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            # print(f"sample feat_id: {feat_id}, feat {feat.shape}")
            B, H, W = feat.shape[0], feat.shape[2], feat.shape[3]
            feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2)
            # print(f"feat reshape: {feat_reshape.shape}")
            if num_patches > 0:
            ...
            # print(f"patch sample: {x_sample.shape}")
...
class GlobalSampleF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(GlobalSampleF, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        B0, H0, W0 = feats[0].shape[0], feats[0].shape[2], feats[0].shape[3]
        ...
        for feat_id, feat in enumerate(feats):
            ...
            resized_feature_map = feat
            if scale_factor > 1:
                resized_feature_map = F.interpolate(feat, scale_factor=scale_factor, mode='bilinear', align_corners=False)

            # print(f"resized_feature_map {resized_feature_map.shape}")
            feat_reshape = resized_feature_map.permute(0, 2, 3, 1).flatten(1, 2)
            # print(f"feat reshape: {feat_reshape.shape}")

            aligned_feats.append(feat_reshape)
        aligned_feats = torch.cat(aligned_feats, dim=2)
        # print(f"aligned feats: {aligned_feats.shape}")

        # aligned = aligned_feats.flatten(0, 1)  # reshape(-1, x.shape[1])
        # print(f"aligned 1: {aligned.shape}")
        if self.use_mlp:
            aligned_feats = self.mlp(aligned_feats)
        # print(f"aligned 2: {aligned_feats.shape}")
        for feat_id, feat in enumerate(feats):
            # print(f"feat_id: {feat_id}, feat {feat.shape}")
            B, H, W = feat.shape[0], feat.shape[2], feat.shape[3]
            scale_factor = W0 // W
            ...
            patch_id_new = (w_id + W0 * h_id) * scale_factor
            # print(f"id1: {patch_id[:5]}, w {w_id[:5]}, h {h_id[:5]}, new {patch_id_new[:5]}")
            # print(f"id2: {(patch_id*scale_factor2)[:10]}")
            patch_id = torch.tensor(patch_id_new, dtype=torch.long, device=feat.device)

            # print(f"id1: {patch_id[:10]}")
            # print(f"id2: {(patch_id*scale_factor2)[:10]}")
            patch_id = torch.tensor(patch_id, dtype=torch.long, device=feat.device)
            x_sample = aligned_feats[:, patch_id, :].flatten(0, 1)  # reshape(-1, x.shape[1])
            x_sample = self.l2norm(x_sample)
            # print(f"x_sample: {x_sample.shape}")
            return_feats.append(x_sample)
        return return_feats
...
class GlobalClusterF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[], km_mode="eu", opt=None):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(GlobalClusterF, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            resized_feature_map = feat
            if scale_factor > 1:
                resized_feature_map = F.interpolate(feat, size=(H0, W0), mode='bilinear', align_corners=False)
            ...
            aligned_feats.append(feat_reshape)
        aligned_feats = torch.cat(aligned_feats, dim=2).flatten(0, 1)
        # print(f"aligned feats: {aligned_feats.shape}")
        ...
        # print(f"aligned 2: {aligned_feats.shape}")
        aligned_feats = self.l2norm(aligned_feats)

        # aligned = aligned_feats.flatten(0, 1)  # reshape(-1, x.shape[1])
        # print(f"aligned 3: {aligned_feats.shape}")

        aligned_labels = self.km(aligned_feats)
        # print(f"aligned labels: {aligned_labels.shape}")
        aligned_labels = aligned_labels.view(B0, -1)
        # print(f"aligned labels2: {aligned_labels.shape}")
        ...
        for feat_id, feat in enumerate(feats):
            ...
            # resized_label_map = aligned_labels
            # print(f"aligned_feats {aligned_labels.shape}")
            # if scale_factor > 1:
            #     resized_label_map = F.interpolate(aligned_labels, size=(B, H, W), mode='nearest')
            #
            # print(f"feat shape: {feat.shape}, label shape: {resized_label_map.shape}")
            # # scale_factor2 = scale_factor**2
            # # print(f"scale_factor2 {scale_factor2}")
            # resized_label_map = resized_label_map.flatten(1, 2)
            # # print(f"label shape: {resized_label_map.shape}")

            assert patch_ids is not None
            patch_id = patch_ids[feat_id]# * scale_factor2
            ...
            # print(f"id1: {patch_id[:5]}, w {w_id[:5]}, h {h_id[:5]}, new {patch_id_new[:5]}")
            # print(f"id2: {(patch_id*scale_factor2)[:10]}")
            patch_id_new = torch.tensor(patch_id_new, dtype=torch.long, device=feat.device)
            x_sample = aligned_labels[:, patch_id_new].flatten(0, 1)  # reshape(-1, x.shape[1])

            # x_sample = aligned_feats[:, patch_id, :].flatten(0, 1)  # reshape(-1, x.shape[1])
            # x_sample = self.l2norm(x_sample)
            # print(f"x_sample: {x_sample.shape}")
            return_labels.append(x_sample)
        return return_labels


class GlobalClusterF2(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[], opt=None):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(GlobalClusterF2, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        aligned_labels = self.km(aligned_feats)
        # print(f"aligned labels: {aligned_labels.shape}")
        aligned_labels = aligned_labels.view(B0, 1, H0, W0)
        # print(f"aligned labels2: {aligned_labels.shape}")
        ...
        for feat_id, feat in enumerate(feats):
            ...
            resized_label_map = aligned_labels.float()
            # print(f"aligned_feats {aligned_labels.shape}, type {resized_label_map.dtype}")
            if scale_factor > 1:
                resized_label_map = F.interpolate(resized_label_map, scale_factor=1./scale_factor, mode='nearest')
            #
            # print(f"feat shape: {feat.shape}, label shape: {resized_label_map.shape}")
            # # scale_factor2 = scale_factor**2
            # # print(f"scale_factor2 {scale_factor2}")
            resized_label_map = resized_label_map.flatten(2, 3)
            # # print(f"label shape: {resized_label_map.shape}")
            ...
            # print(f"id2: {(patch_id*scale_factor2)[:10]}")
            patch_id = torch.tensor(patch_id, dtype=torch.long, device=feat.device)
            x_sample = resized_label_map[:, :, patch_id].flatten(0, 1).int()  # reshape(-1, x.shape[1])

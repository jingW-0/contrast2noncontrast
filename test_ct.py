...
def save_result(path, save_path, model, opt):
    ...
    recon_image.SetSpacing(image.GetSpacing())
...
def series_gen_heart(opt, from_folder, result_folder, netG):
    ...
    for file in os.listdir(from_folder):
        path_A = os.path.join(from_folder, file)
        save_path = os.path.join(result_folder, file)
        # if opt.direction == "AtoB":
        save_result(path_A, save_path, netG, opt)
...
if __name__ == '__main__':
    ...
    result_dir = "D:/heart_segmentation/ce2nc_segtest"

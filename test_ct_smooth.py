...
def save_result(path, save_path, model, opt):
    ...
    print(f"shape1: {image_array.shape}")
    # Resize the image
    image_array = scipy.ndimage.zoom(image_array, [512/ image_array.shape[0], 512/image_array.shape[1], 1], order=3)  # order=3 for cubic interpolation
    print(f"shape2: {image_array.shape}")
    ...
    recon = scipy.ndimage.zoom(recon, [1, original_shape[0]/512, original_shape[1]/512], order=3)  # order=3 for cubic interpolation
...
def apply_gaussian_window(patch, sigma=1):
    """Apply a Gaussian window to the patch to weight the center more."""
...
def inference(model, image, patch_size_x, patch_size_y, stride, batch_size=1):
    ...
    for i in range(len(batches)):
        ...
        label_np[istart:iend, jstart:jend] += apply_gaussian_window(pred)
        weight_np[istart:iend, jstart:jend] += apply_gaussian_window(np.ones_like(pred))


        # plt.imshow(apply_gaussian_window(np.ones_like(pred)), cmap='gray')
...
if __name__ == '__main__':
    ...
    if opt.direction == "AtoB":
        result_dir = "D:\\contrast\\project2\\result3\\" + opt.name + "\\AtoB"
    else:
        result_dir = "D:\\contrast\\project2\\result3\\" + opt.name + "\\BtoA"
    ...
    # else:
    #     result_dir = "D:\\contrast\\project2\\result_heart\\" + opt.name + "\\BtoA"
    # result_dir = "D:/heart_segmentation/ce2nc_val"
    # if not os.path.exists(result_dir):
    #     os.makedirs(result_dir)

...
class ImageDatasetFromFile(Dataset):
    def __init__(self, pathA, pathB, transforms_1=None, unaligned=True, max_itr=1, with_metric=None, metric_tol=np.inf, ssim_th=0):
        self.transform1 = transforms.Compose(transforms_1)
    ...
    def __getitem__(self, index):
        ...
        best_path_B = ""
        if self.unaligned:
            if self.with_metric is None:
                path_B = self.files_B[random.randint(0, len(self.files_B) - 1)]
            elif self.with_metric == 'ssim':
                img_A_ = scipy.ndimage.interpolation.zoom(img_A, [64 / img_A.shape[0],
                                                                  64 / img_A.shape[1]], order=1)
                nsampling = 0
                prev_ssim = 0
                while nsampling < self.max_itr:
                    ...
                    if s > self.ssim_th:
                        break
            elif self.with_metric == 'metric':
                metric_A = self.metrics_A[index % len(self.files_A)]
                nsampling = 0
                prev_metric_diff = np.inf
                while nsampling < self.max_itr:
                    ...
                    abs_diff = np.abs(metric_A - metric_B)
                    if abs_diff < prev_metric_diff:
                        prev_metric_diff = abs_diff
                        best_path_B = path_B

                    if abs_diff < self.metric_tol:
                        break
            else:
                raise ValueError("--with metric should be 'ssim', 'metric', or None")
        else:
